...
class Pagination
{
    ...
    private $class_buttons = 'btn btn-default';
    private $class_buttons_active = 'btn btn-primary';
    

    public function __construct($url, $current_page = 1, $table_rows_count = 0, $rows = 10, $printed_pages = 10)
    {
...
//------------------------------------------------------------------------
    ...
    private function get_current_page_html($name)
    {
        ...
        return '<span class="'.$this->class_buttons_active.'">' . $name . '</span>';
    }
    ...
    private function get_go_button_html()
    {
        ...
        if ($this->show_go_button)
            $this->output .= '<script>
                ...
                </script><span class="">' . $this->page_text . ' : <input class="input-group-addon" onkeydown="return keypress_' . $id . '(event)" type="text" name="' . $id . '" id="' . $id . '" size="3" value="' . $this->current_page . '"  ></span>
                <a class="'.$this->class_buttons_active.'" onclick="fun_' . $id . '()" href="#">' . $this->go_button_text . '</a>';
    }
...
//------------------------------------------------------------------------
    ...
    private function get_pages_number_html()
    {
        if ($this->show_pages_number) {
            $this->output .= '<span class="'.$this->class_buttons_active.'">' . $this->current_page . ' ' . $this->pages_number_text . ' ' . $this->pages_count . '</span>';
        }
    }
