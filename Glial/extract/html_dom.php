...
class html_dom {
    ...
    public static function stripTags($html, $stripTags)
	{
		if (isset($stripTags))
		{
			if (is_array($stripTags))
			{
				foreach ($stripTags as $sTag)
				{
					$sTagContentsFull = self::getTagContents($content, $sTag, false);
					$sTagContentsStripped = self::getTagContents($content, $sTag, true);
					$content = str_replace($sTagContentsFull, $sTagContentsStripped, $content);
				}
			}
			else
			{
				$sTagContentsFull = self::getTagContents($content, $stripTags, false);
				$sTagContentsStripped = self::getTagContents($content, $stripTags, true);
				$content = str_replace($sTagContentsFull, $sTagContentsStripped, $content);
			}
		}
		else
    ...
    public static function removeTags($html, $tag_to_remove)
	{
		if (isset($tag_to_remove))
		{
			if (is_array($tag_to_remove))
			{
				foreach ($tag_to_remove as $rTag)
				{
					$rTagContents = self::getTagContents($content, $rTag);
					$content = str_replace($rTagContents, '', $content);
				}
			}
			else
			{
				$rTagContents = self::getTagContents($content, $tag_to_remove);
				$content = str_replace($rTagContents, '', $content);
			}
		}
		else
