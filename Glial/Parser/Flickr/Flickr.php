...
//16947, 11389, 14637, 14470, 15961, 
/*
 * UPDATE b.* FROM scientific_name_translation b
inner join `species_picture_search` a ON a.id_species_main = b.id_species_main AND a.language = b.language AND b.`text` =a.tag_search
SET is_black_list = 1
where a.total_found > 1783 and a.id not in (16947, 11389, 14637, 14470, 15961)
order by a.total_found desc
 */
//1782
...
class Flickr
{
    ...
    public static function getLinksToPhotos($query)
    {
        ...
        for ($i = 1; $i < 67; $i++) { // 67 pages is a max of Flickr
           ...
           //http://www.flickr.com/search/?q=timor+sparrow&ss=0&ct=0&mt=photos&w=all&adv=1
           //http://www.flickr.com/search/?q=%22timor+sparrow%22&ss=0&ct=0&mt=all&w=all&adv=1
            ...
            foreach ($contents as $var) {
                ...
                if (preg_match($pattern, trim($img), $mathes)) {
                    ...
                    debug($generate_url);
                    debug($var);
                    continue; // because must be registered to see horror photos
                }
                ...
                $ret['author']['name'] = trim($author);

                $tab_key = explode('/', $ret['url']);
                $ret['author']['key'] = $tab_key[4];
