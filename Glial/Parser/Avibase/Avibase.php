...
class Avibase
{
    ...
    public static function get_species_by_reference($id_avibase = "01A3BE3CBE9C7B39")
    {
        ...
        $content2 = Grabber::getTagContents($content, '<td class="AVBContainerText" bordercolor="#FFFFFF" width=50%>', true);
        if (false === $content2) {
            return false;
        }
        ...
        $p = Grabber::getTagContents($content2[0], '<p>', true);
        foreach ($p as $p_part) {
            $ret = self::extract_p($p_part);
            $data = array_merge($data, $ret);
        }
        $data['TSN'] = Grabber::getTagContent($p[2], '<a', true);

        $ret = self::extract_p($content2[1]);
        $data['Language'] = $ret;

        $ret = Grabber::getTagContent($content, '<td class="AVBSynonyms">', true);
        ...
        foreach ($syn as $tab_syn) {
            $var = trim(str_replace(":", "", Grabber::getTagContent($tab_syn, '<b>', true)));
        ...
        $ret = Grabber::getTagContent($content, '<span class="AVBContainerText">', true);
    ...
    public static function extract_p($p)
    {
        $p = trim($p);
        ...
        $title = Grabber::getTagContents($p, '<b>', true);
    ...
    public static function get_regions()
    {
        ...
        $data = array();
        $content2 = \Grabber::getTagContent($content, '<table border="0" width="860" cellspacing="1">', true);
    ...
    public static function get_ids($region)
    {
        ...
        $data = array();
        $content2 = \Grabber::getTagContent($content, '<table class="AVBlist">', true);
