...
class OiseauxNet
{
    public static function get_species_from_family($famliy = "estrildides")
    {
        ...
        $content2 = Grabber::getTagContent($content, '<table class="tb_lite">', true);
        if (false === $content2) {
            return false;
        }

        $tab_tr = Grabber::getTagContents($content2, '<tr', true);
        ...
        foreach ($tab_tr as $tr) {
            ...
            $out['French'] = Grabber::getTagContent($tr, '<a href="', true);
            $url = \wlGrabber::getTagContent($tr, '<a href="', false);
            ...
            $tab_td = Grabber::getTagContents($tr, '<td', true);
