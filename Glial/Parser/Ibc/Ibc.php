...
class Ibc
{
    ...
    public static function getSpeciesFromFamily($famliy = "waxbills-estrildidae")
    {
        ...
        $content2 = Grabber::getTagContent($content, '<ul id="media-status-specie" class="media-status-specie">', true);
        if (false === $content2) {
            return false;
        }
        $tab_li = Grabber::getTagContents($content2, '<li', true);

        foreach ($tab_li as $li) {
            $out = array();

            $url = Grabber::getTagContent($li, '<a href="/species/', false);
            ...
            $out['scientific_name'] = Grabber::getTagContent($li, '<span class="scientific">', true);
    ...
    public static function getOrderAndFamily()
    {
        ...
        $data = array();
        $content2 = Grabber::getTagContent($content, '<div class="order-content">', true);
        $data_order = Grabber::getTagContents($content, '<ul class="menu" id="', true);

        foreach ($data_order as $order) {
            $ordername = Grabber::getTagContent($order, '<a>', true);
            $li = Grabber::getTagContents($order, '<ul style="display: none;">', true);

            foreach ($li as $family) {
                $english = Grabber::getTagContent($family, '<strong>', true);
                $url = explode('"', $family);
    ...
    public static function getPhotoAndInfos($picture_link)
    {
        ...
        $data = array();
        $content2 = Grabber::getTagContent($content, "<h4>", true);
        $content2 = Grabber::getTagContent($content2, "<a", true);
        $family_name = preg_split("#[\(\)]+#", $content2);
        $data['family_name'] = $family_name[0];
        $data['family_english'] = $family_name[1];
        $content2 = Grabber::getTagContent($content, "<h3><a", true);
        $name = preg_split("#[\(\)]+#", $content2);
        $data['name'] = $name[0];
        $data['scientific_name'] = Grabber::getTagContent($name[1], "<i>", true);
        $content2 = Grabber::getTagContent($content, '<ul class="status">', true);
        $data['status'] = Grabber::getTagContent($content2, '<li', true);
        $content2 = Grabber::getTagContent($content, '<div class="photo-frame">', true);
        $photo = Grabber::getTagContent($content2, '<img', false);
        $photo = preg_split("#[\"]#", $photo);
        $data['photo_url'] = $photo[1];
        $data['legend'] = Grabber::getTagContent($content2, '<div class="caption-photo"', true);
        $data['recorded'] = date("Y-m-d", strtotime(Grabber::getTagContent($content, '<span class="date-display-single">', true)));
        $data['uploaded'] = date("Y-m-d", strtotime(Grabber::getTagContent($content, '<em>', true) . 'ago'));
        ...
        $content2 = Grabber::getTagContent($content, '<div id="gmaps">', true);
        $content2 = Grabber::getTagContent($content2, '<script>', true);
        $coords = preg_replace('#.+var coordX = \'(.+)\';.+var coordY = \'(.+)\';.+#isU', '$1 ($2)', $content2);
        $coords = preg_split('#[\(\)]#', $coords);
        $data['longitude'] = $coords[0];
        $data['latitude'] = $coords[1];
        $data['ranking'] = Grabber::getTagContent($content, '<div class="star star-1 star-odd star-first"><span class="on">', true);
    ...
    public static function getVideoAndInfos($video_link)
    {
        ...
        $content2 = Grabber::getTagContent($content, '<ul class="status">', true);
        $data['status'] = Grabber::getTagContent($content2, '<li', true);
        $content2 = Grabber::getTagContent($content, '<div class="video-frame">', true);
        $video = Grabber::getTagContent($content2, '<script', true);
        $video = explode('"', $video);
        $video = explode('\x', $video[19]);
        $video = explode('=', $video[0]);
        $data['video_url'] = 'http://ibc.lynxeds.com/files/videos/transcoded/' . $video[1] . '.mp4';
        $data['legend'] = Grabber::getTagContent($content2, '<div class="caption-video"', true);
        $data['recorded'] = date("Y-m-d", strtotime(Grabber::getTagContent($content, '<span class="date-display-single">', true)));
        $data['uploaded'] = date("Y-m-d", strtotime(Grabber::getTagContent($content, '<em>', true) . 'ago'));
        $author = explode('<a href="/users/', $content);
        $author = explode('</a>', $author[1]);
    ...
    public static function getSoundAndInfos($sound_link)
    {
        ...
        $data['legend'] = Grabber::getTagContent($content, '<div class="caption-sound">', true);
        $data['recorded'] = date("Y-m-d", strtotime(Grabber::getTagContent($content, '<span class="date-display-single">', true)));
        $data['uploaded'] = date("Y-m-d", strtotime(Grabber::getTagContent($content, '<em>', true) . 'ago'));
        $author = explode('<a href="/users/', $content);
        $author = explode('</a>', $author[1]);
    ...
    public static function getSpeciesInfosAndLinks($link)
    {
        ...
        $content2 = Grabber::getTagContent($content, '<div class="hovertip" id="all_tax">', true);
        ...
        /* $content2 = Grabber::getTagContent($content, '<div class="hovertip" id="all_ssp"><div class=\'view view-subspecies-in-species\'><div class=\'view-content view-content-subspecies-in-species\'><div class="item-list"><ul>', true);
    ...
    public static function getUserInfos($link)
    {
        ...
        $data = array();
        $pic = explode('"', Grabber::getTagContent($content, '<div id="image">', true));
        $data['user_picture_url'] = 'http://ibc.lynxeds.com' . $pic[1];
        $name = explode('|', Grabber::getTagContent($content, '<title>', true));
        $data['username'] = trim($name[0]);
        $data['from'] = trim(str_replace('<br>', ', ', Grabber::getTagContent($content, '<h4>', true)));
        $locations = Grabber::getTagContent($content, '<div class="puntitos-block">');
        ...
        $infos = Grabber::getTagContent($content, '<div class="info">', true);
        $infos = explode('</p>', $infos);
        $infos = explode('<strong>', $infos[2]);
        $data['biography'] = trim(strip_tags($infos[0]));
        $data['first_posted'] = date('Y-m-d', strtotime(Grabber::getTagContent(Grabber::getTagContent($content, '<li id="first-posted">', true), '<span>', true)));
        $videos_posted = Grabber::getTagContent($content, '<li id="videos-posted">', true);
        ...
        $images_posted = Grabber::getTagContent($content, '<li id="images-posted">', true);
        ...
        $sounds_posted = Grabber::getTagContent($content, '<li id="sounds-posted">', true);
