...
namespace Glial\Scheduler\Background;

class Background
{

    static public function backgroundExec($function_name, $params, $str_requires, $timeout = 600)
    {
        $map = array('"' => '\"', '$' => '\$', '`' => '\`', '\\' => '\\\\', '!' => '\!');
        ...
        $my_background_exec = "(/usr/bin/php -r \"chdir('{$path_run}');{$str_requires} Background::timeOutExec(\\\"{$my_target_exec}\\\", file_get_contents('php://stdin'), {$timeout});\" <&3 &) 3<&0"; //php by default use "sh", and "sh" don't support "<&0"
        self::timeOutExec($my_background_exec, json_encode($params), 2);
    }

    static public function timeOutExec($cmd, $stdin = '', $timeout)
    {
        $start = time();
        $stdout = '';
        $stderr = '';
