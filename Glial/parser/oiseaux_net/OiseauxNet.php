...
class OiseauxNet {
	...
	static function get_species_from_family($famliy = "estrildides") {
		...
		$content2 = \wlHtmlDom::getTagContent($content, '<table class="tb_lite">', true);
		if (false === $content2)
		{
			return false;
		}
		
		$tab_tr = \wlHtmlDom::getTagContents($content2, '<tr', true);
		...
		foreach($tab_tr as $tr)
		{
			...
			$out['French'] = \wlHtmlDom::getTagContent($tr, '<a href="', true);
			...
			$tab_td = \wlHtmlDom::getTagContents($tr, '<td', true);
