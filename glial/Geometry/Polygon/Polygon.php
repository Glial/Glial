...
use \Glial\Geometry\Polygon\Vertex; // A polygon consists of vertices. So the polygon
...
class Polygon
{
    ...
    public function polygon($first = NULL)
    {
        $this->first = $first;
        $this->cnt = 0;
    }
...
// whose alpha is lower but don't go past
    ...
    function &first_unckd_intersect() {
        $v = & $this->first;
        do		 // Do-While {		 // Not yet reached end of the polygon
    ...
    public function angle($xc, $yc, $x1, $y1)
    {
    ...
    public function ints(&$p1, &$p2, &$q1, &$q2, &$n, &$ix, &$iy, &$alphaP, &$alphaQ)
    {
        $found = FALSE;
...
//					print("Perturb: P(".$p1->X().",".$p1->Y().")(".$p2->X().",".$p2->Y().") Q(".$q1->X().",".$q1->Y().")(".$q2->X().",".$q2->Y().") UA(".$ua.") UB(".$ub.")<br>");
        ...
        elseif ($pt != 0 && $qt != 0) { // Is  it Arc/Arc?
            ...
            if ($d == 0) {  // Don't try an find intersection if centers are the same.	 // Added in Rev 1.2
                $found = FALSE;
            } elseif ($d > ($r0 + $r1)) {  // Check for solvability.		// no solution. circles do not intersect.
                $found = FALSE;
             ...
             */
            if ($pt == 0) { // Segment p1,p2 is the line	// Segment q1,q2 is the arc
                $x1 = $p1->X();
                $y1 = $p1->Y();
                $x2 = $p2->X();
                $y2 = $p2->Y();
                ...
                $ye = $q2->Y();
                $d = $qt;
            } else {   // Segment q1,q2 is the line	// Segment p1,p2 is the arc
                $x1 = $q1->X();
                $y1 = $q1->Y();
                $x2 = $q2->X();
                $y2 = $q2->Y();
...
// end of boolean function
    ...
    public function isPolyInside(&$p)
    {
        $inside = TRUE;
            ...
            do {
                ...
                do {
                    if ($v->d() != 0) { // Is it an arc segment
                        ...
                        } else { // There was no intersection so bounding rect is determined  // by the start point only, not the edge of the arc
                            $minY = min($minY, $v->Y());
                            $maxY = max($maxY, $v->Y());
                        }
                        $v1 = NULL;
